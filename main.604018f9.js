...
var entityScan = document.querySelector(".entityScan"); // dialogs

exports.entityScan = entityScan;
var drawpile = document.querySelector(".drawPile");
exports.drawpile = drawpile;
var discardpile = document.querySelector(".discardPile");
exports.discardpile = discardpile;
var drawpileDialog = document.querySelector(".drawPile-dialog");
exports.drawpileDialog = drawpileDialog;
drawpileDialog.drawCardGrid = drawpileDialog.querySelector(".card-grid");
var discardpileDialog = document.querySelector(".discardPile-dialog");
...
var basic_attack_friendly = /*#__PURE__*/function () {
  ...
  _createClass(basic_attack_friendly, [{
    ...
    value: function exe() {
      ...
      (0, _functions.renderEntityAttack)(this.entity, this);
    }
  }, {
    key: "render",
...
var basic_deffend = /*#__PURE__*/function () {
  ...
  _createClass(basic_deffend, [{
    ...
    value: function exe() {
      ...
      (0, _functions.renderEntitySkill)(this.entity, this);
    }
  }, {
    key: "render",
    value: function render() {
...
exports.playerDeckInit = playerDeckInit;
...
function playerDeckInit() {
  _player.player.deck = [];

  _player.player.actualDeck.forEach(function (card) {
    var cloneCard = Object.assign(Object.create(Object.getPrototypeOf(card)), card); // because the class itself doesn't generate object element we have to reassign it our self

    card.element.cardObject = cloneCard;

    _player.player.deck.push(cloneCard);
  });
}
...
exports.determineCardType = determineCardType;
exports.displayDiscardDialog = displayDiscardDialog;
exports.displayDrawDialog = displayDrawDialog;
...
function generateEntityScan(entity) {
  ...
  for (var effect in entity.buffs) {
    var effectOb = entity.debuffs[effect];
    var effectElement = document.querySelector(".effect_description.".concat(effect)).cloneNode(true);
    replaceInWith(effectElement, ".uses", effectOb.uses);
    replaceInWith(effectElement, ".duration", effectOb.duration);
  ...
  for (var _effect in entity.debuffs) {
    var _effectOb = entity.debuffs[_effect];

    var _effectElement = document.querySelector(".effect_description.".concat(_effect)).cloneNode(true);

    replaceInWith(_effectElement, ".uses", _effectOb.uses);
    replaceInWith(_effectElement, ".duration", _effectOb.duration);
...
function replaceInWith(parrent, childSelector, text) {
  var element = parrent.querySelector(childSelector);
  if (!element) return;
...
function closeModal(e) {
  ...
  var modal = closeBtn.parentElement;
...
function displayDrawDialog() {
  var dialog = dom.drawpileDialog;
  renderCardGrid(dialog.drawCardGrid, _player.player.drawPile);
  dialog.showModal();
}

function displayDiscardDialog() {
  var dialog = dom.discardpileDialog;
  renderCardGrid(dialog.discardCardGrid, _player.player.discardPile);
  renderCardGrid(dialog.graveCardGrid, _player.player.grave);
  dialog.showModal();
}

function renderCardGrid(cardGrid, pile) {
  cardGrid.innerHTML = "";
  var card_array = Array.from(pile).sort(function (cardA, cardB) {
    return cardA.name > cardB.name ? 1 : cardA.name < cardB.name ? -1 : 0;
  });
  card_array.forEach(function (card) {
    var element = card.element.cloneNode(true);
    cardGrid.append(element);
  });
}
...
exports.renderEntityAttack = renderEntityAttack;
exports.renderEntityScan = renderEntityScan;
exports.renderEntitySkill = renderEntitySkill;
...
function renderEntityAttack(entity, moveObj) {
  var modal = entity.modal;
  var animation_name;

  if (moveObj.animatation) {
    animation_name = moveObj.animatation;
  } else {
    animation_name = "attacking";
  }

  modal.classList.add(animation_name);
  setTimeout(function () {
    modal.classList.remove(animation_name);
  }, 3000);
}

function renderEntitySkill(entity, moveObj) {
  var modal = entity.modal;
  var animation_name;
...
function playCardManager(card, entity) {
  ...
  card.play(entity);
  domfunctions.renderPlayerEnergy(); // player animation
...
// new skillCardClasses.defend(),
new skillCardClasses.defend(), new skillCardClasses.defend(), new attackCardClasses.bash()]; // const attak1 = new attackCardClasses.strike()
// const attak2 = new attackCardClasses.strike()
...
function battleInit() {
  ...
  f.cardfunctions.playerDeckInit();
  f.cardfunctions.playerDeckInit();
...
//# sourceMappingURL=main.604018f9.js.map